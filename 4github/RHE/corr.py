...
class CorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4, sz=32):
        self.num_levels = num_levels                                                    # num_levels其实输入的是2，应该代表的是几个尺度
        self.radius = radius                                                            # 扰动范围
        self.corr_pyramid = []

        corr = CorrBlock.corr(fmap1, fmap2,sz)                                          # 算相关关系
        batch, h1, w1, dim, h2, w2 = corr.shape
        corr = corr.reshape(batch * h1 * w1, dim, h2, w2)                               # dim就是1

        self.corr_pyramid.append(corr)                                                  # 这个是正常的corr
        for i in range(self.num_levels - 1):
            corr = F.avg_pool2d(corr, 2, stride=2)                                      # 对应那个后两个维度变成原来的一半
            self.corr_pyramid.append(corr)                                              # 把这哥俩放在一起了
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            dx = torch.linspace(-r, r, 2 * r + 1)                                       # 正方形小块的范围
            dy = torch.linspace(-r, r, 2 * r + 1)
            delta = torch.stack(torch.meshgrid(dy, dx), axis=-1).to(coords.device)

            centroid_lvl = coords.reshape(batch * h1 * w1, 1, 1, 2) / 2 ** i
            delta_lvl = delta.view(1, 2 * r + 1, 2 * r + 1, 2)
            xgrid, ygrid =  delta_lvl.split([1, 1], dim=-1)

            coords_lvl = centroid_lvl + delta_lvl                                       # 这里在算采的地方
...
class TransCorrBlock(CorrBlock, nn.Module):
    def __init__(self, config, num_levels=2, radius=4, do_corr_global_norm=False):
