...
class homo_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img3 = cv2.imread(self.image_list_img3[index],cv2.IMREAD_GRAYSCALE)

        if self.dataset=='mscoco':
            img1 = cv2.resize(img1, (320, 240))
            img2 = cv2.resize(img2, (320, 240))
            e1 = cv2.resize(img3, (320, 240))

        e1 = np.expand_dims(e1,axis=-1)

        img1= np.concatenate((img1, e1), axis=2)
        img2 = np.concatenate((img2, e1), axis=2)
        ...
        img_patch_ori1 = img_patch_ori.copy()
        img_patch_pert1 = img_patch_pert.copy()
        ...
        img_patch_ori1 = img_patch_ori1[:, :, :3]
        img_patch_pert1 = img_patch_pert1[:, :, :3]

        img1_g = self.transform(img_patch_ori1)
        img2_g = self.transform(img_patch_pert1)
        ...
        return img2, img1, flow, H ,img2_g, img1_g
...
class MYDATA(homo_dataset):
    def __init__(self, split='train', dataset='msococo'):
        ...
        if split == 'train':
            if dataset=='mscoco':
                root_img1 = './dataset/train2017/train2017'
                root_img2 = './dataset/train2017/train2017'
                root_img3 = './dataset/train2017/train2017pdn0'
            if dataset=='ggmap':
            ...
            if dataset=='mscoco':
                root_img1 = './dataset/test2017/test2017'
                root_img2 = './dataset/test2017/test2017'
                root_img3 = './dataset/test2017/test2017pdn'
            if dataset=='ggmap':
        ...
        self.image_list_img3 = sorted(glob(osp.join(root_img3, '*.png')))
...
def fetch_dataloader(args, split='train'):
    ...
    if split == 'train':
        ...
        train_dataset = MYDATA(split='val',dataset='mscoco')
